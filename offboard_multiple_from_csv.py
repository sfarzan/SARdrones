...
async def initial_setup_and_connection(drone_id, udp_port, grpc_port):
    if NO_COORDINATOR==True:
        grpc_port = grpc_port + drone_id
    home_position = None
...
async def create_drone_configurations(num_drones, time_offset):
    ...
    if NO_COORDINATOR == True:
        ...
        udp_ports = []
        udp_ports.append(params.mavsdk_port)
...
def start_mavsdk_servers(num_drones, udp_ports):
    ...
    for i in range(num_drones):
        if NO_COORDINATOR == True:
            port = params.grpc_port + i
        else:
            port = params.grpc_port
...
async def run_drone(drone_id,home_position_NED, trajectory_offset, udp_port, time_offset, altitude_offset):
    if NO_COORDINATOR == False:
        drone_id = params.hw_id
    # Call the initial setup and connection function
    drone, mode_descriptions, home_position = await initial_setup_and_connection(drone_id, udp_port, params.grpc_port)
    ...
    if SEPERATE_CSV:
        filename = "../FlightPaths/swarm/processed/Drone " + str(drone_id) + ".csv"    
    else:
