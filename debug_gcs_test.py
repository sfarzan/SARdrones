...
try:
    for drone_config in drones:
        ...
        endpoints.append(f"-e {drone_config.config['gcs_ip']}:{params.mavsdk_port + drone_config['hw_id']}")

        #Append Connection for GCS Script
        endpoints.append(f"-e 127.0.0.1:{params.comms_port + drone_config['hw_id']}")
...
try:
    for drone_config in drones:
        ...
        # Pymavlink Connection for communication
        master = mavutil.mavlink_connection
        
        # Pymavlink Connection
        try:
            master = mavutil.mavlink_connection(f'udp:localhost:{Pymavlink_Port}')
        except Exception as e:
