...
class DroneCommunicator_HW:
    ...
    async def connect_to_drone(self):
        print('CONNECTING COMMS')
        # await self.comms.connect(system_address='udp://:14540')
        await self.comms.connect(system_address=f'udp://:{self.params.comms_port}')
        print('Waiting for Drone Communicator to Connect...')
        async for state in self.comms.core.connection_state():
            if state.is_connected:
                print('Communicator Connected to Drone')
                self.connected = True
                break

    async def send_telem(self):
        print("send_telem")
        if self.connected == True:
            await self.comms.server_utility.send_status_text(
                StatusTextType.INFO, 'Work Please')

    def send_packet_to_node(self, packet):
        if self.connected == True:
            asyncio.run(self.comms.server_utility.send_status_text(
                StatusTextType.INFO, packet))
    ...
    def process_packet(self, data):
        if data is not None and data.get_type() == 'STATUSTEXT':
    ...
    def read_packets(self):
        if self.connected == True:
            self.comms.telemetry.subscribe_status_text(self.process_packet)
            while True:
                time.sleep(1)
